...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    const destinationPath = path.join(__dirname, '..', 'public', 'reimbursement_transaction');
...
router.post('/reimbursement', upload.single("transaction_receipt"), (req, res) => {
  try {
    const { date, employee_id, type_id, amount, note, status, reason } = req.body;
...
router.patch('/reimbursement/:id', upload.single("transaction_receipt"), (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      mysqlConnection.query('UPDATE reimbursement SET date = ?, employee_id = ?, type_id = ?, amount = ?, note = ?, status = ?, reason = ? WHERE id = ?',
        [date, employee_id, type_id, amount, note, status, reason, req.params.id],
        (err, rows, fields) => {
          if (!err) res.send('Reimbursement updated successfully');
          else console.log(err);
        });
